...
class MCT:
    ...
    def backtrack(self, cur, reward, root, train):
        while cur.state != root.state:
            ...
            cur = cur.parent
            if train:
                #print('back track add datapoint', {'state':cur.state,'Q':cur.Q,'P':cur.P})
    ...
    def test(self,times=20,iter=50):
        ...
        earned = sum(earned)/times
        print(f'final avg earned: {earned}')
        return earned
...
def plot(reward, mv_reward, losses,test_earned):
    ...
    plt.legend()
    plt.plot(test_earned,label='test earned')
    plt.legend()
    plt.show()
...
def simulate(new_model, training_itr, deck_num, search_amount, explore_constant):
    ...
    for _ in range(training_itr):
        ...
        print(len(tree.dataset.data), ' dataset length  ',tree.dataset.new_count, ' new count ')
        if tree.dataset.new_count >= 100 and len(tree.dataset) >= 500:
            loss_list,earned = tree.train()
            losses.extend(loss_list)
            test_earned.append(earned)
...
if __name__ == '__main__':
    simulate(new_model=0, training_itr=5000, deck_num=6, search_amount=50, explore_constant=0.1)