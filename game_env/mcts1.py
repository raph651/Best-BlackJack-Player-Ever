...
class Node:
    def __init__(self, state, parent, prior_action, network, child=None):
        ...
        if state:
            self.P, self.v = self.network(torch.FloatTensor([state]))
            self.P = self.P[0].tolist()
            self.v = self.v[0].item()
...
class MCT:
    ...
    def search(self, root):
        ...
        for _ in range(self.search_amount):
            ...
            action = self.get_good_action(root) # self.search_then_refine(cur, _, thre=0.15)
        ...
        self.env = temp_env
        self.dataset.add({'state':root.state,'Q':root.Q,'P':[n/sum(root.N) for n in root.N]})
    ...
    def run(self, root):
        ...
        while True:
            ...
            print(self.env.state.input(), action, " prob_net: ", cur.P, " prob_real: ",
                  [n/sum(cur.N) for n in cur.N], " q: ", cur.Q)
            reward, done = self.env.step(action)
            if done:
    ...
    def backtrack(self, cur, reward, root, train):
        while cur.state != root.state:
            ...
            if train:
                self.dataset.add({'state':cur.state,'Q':cur.Q,'P':[n/sum(cur.N) for n in cur.N]})
...
if __name__ == '__main__':
    simulate(new_model=0, training_itr=10000, deck_num=6, search_amount=10, explore_constant=3)
